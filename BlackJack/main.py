...
#####################  모듈들 ########################
import random        # 랜덤 모듈
from art import logo  # 아트.py파일에서 로고만 가져온다.
...
def deal_card():
    """카드리스트에서 랜덤 카드한장 리턴"""
    cards = [11, 2, 3, 4, 5, 6, 7, 8, 9, 10, 10, 10, 10]
    card = random.choice(cards)  # 리스트에서 랜덤으로 한장리턴
    return card
...
def calculate_score(cards):
    ...
    if sum(cards) == 21 and len(cards) == 2:  # 입력한 카드리스트의 합이 21이고 카드가 2장일때
        return 0                             # 블랙잭
    if 11 in cards and sum(cards) > 21:  # 에이스(11) 카드가 있고 합이 21이 초과할경우
        cards.remove(11)
        cards.append(1)
    return sum(cards)
...
def play_game():
    ...
    while not is_game_over:  # 게임오버가 되지 않았으면 계속 진행
        ...
        print(f' 컴의 첫카드: {com_cards[0]}')
        # 블랙잭 확인과 21초과이면 게임 종료
